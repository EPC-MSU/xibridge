...
bool test_connect_1()
{
    ...
    err = xibridge_open_device_connection("xi-net://172.16.130.28/1f50", &conn);
    if (err)
    {
    ...
    xibridge_enumerate_adapter_devices("xi-net://172.16.130.28/1f50", "", &pdata, &count);
    ZF_LOGD("Count of enumerated devices: %u", count);
    if (count)
    {
        ZF_LOGD("First enumerated device URI: %s", pdata);
    }
