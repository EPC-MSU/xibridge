...
есть в архиве с релизом сервера под все платформы. Чтоб запустить сервер на windows, нужно взять каталог win32 или win64 в архиве с релизом сервера и просто 
запустить приложение `xibridge_server <опции>`  из этого каталога. Для запуска на линукс debian 64 или debian mipsel сначала взять из  архива libsearialport_release 
пакет `libserialport.deb` (в каталоге платформы применения) и установить его так: 
```
* `cd <путь до пакета>`
* `apt install ./libserialport.deb`
```
...
Для запуска сервера на Mac OS нужно: скопировать релиз библиотеки `libserialport.0.dylib` в системную папку `/usr/local/lib`, взять релиз сервера, разрешить в 
системных настройках выполнение для сервера и скопированной библиотеки. Под Mac OS допустимые опции по способу идентификации устройств - `by_serial` и `by_serialpidvid`.
