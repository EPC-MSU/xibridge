...
public:
   ...
   _id_vid(vid, lit_end) 
    ...
    void 
    get_stream(MBuf& mbuf);
    
    void 
    put_stream(MBuf& mbuf) const ;
...
public:
    ...
    // в начало
    bool 
    meminsert_start(const uint8_t *what, 
                    size_t len);
    // вставляет два байта в начало и пересапр. внутр. буфер
    bool 
    meminsert_start(uint8_t num1, 
                    uint8_t num2);
    ...
    size_t 
    restOfSize(int from_pos) const;
        
    uint32_t 
    CRC32(int from_pos);
    
    bvector 
    to_vector(bool rest = false) const;
...
extern bvector 
&add_uint32_2_bvector(bvector & bv, 
                      uint32_t val);

extern bool 
xi_net_dev_uris(MBuf& result, 
                const char *server, 
                const bvector& data_devid, 
                int count);

extern bvector &
add_dev_id_bvector_net_order(bvector & bv, 
